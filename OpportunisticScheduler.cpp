...
int main(int argc, char** argv)
{
    ...
    char outFileName[30] = "Opportunistic_160-";
    char cSeed[11];
//    strcpy(outFileName, "Indexing-");
    ...
    //Pure opportunistic scheduling with no intelligence whatsoever.
    ...
    for(int k=0;k<100000;k++)
    {
        ...
        userBeingScheduled = 0;
        for(int i=1; i<10; i++)
        {
            if(userArray[i].getDataRate(0)>userArray[userBeingScheduled].getDataRate(0)&&userArray[i].getQueueLength()) userBeingScheduled = i;
        }
        userArray[userBeingScheduled].transmitData(schedulingArray1);
        userBeingScheduled = 0;
        for(int i=1; i<10; i++)
        {
            if(userArray[i].getDataRate(1)>userArray[userBeingScheduled].getDataRate(1)&&userArray[i].getQueueLength()) userBeingScheduled = i;
        }
