...
int main(int argc, char** argv)
{
    ...
    char outFileName[40] = "Opp_Indexing_multiplicative_160-";
    char cSeed[11];
//    strcpy(outFileName, "Indexing-");
    ...
    for(int k=0;k<100000;k++)
    {
        ...
        for(int i=0; i<NUM_CHANNELS; i++)
        {
            ...
            {
                delta[j][i] = static_cast<float>(userArray[j].getDataRate(i)) * exp(static_cast<float>(userArray[j].getQueueLength())/50) * exp(-static_cast<float>(userArray[j].getHoQTTL())/20);
            }
        }
...
//End of Opportunistic Scheduler
